...
Camera::Camera(glm::vec3 camera_position, glm::vec3 look_at, glm::vec3 up_vec)
    : cam_pos(std::move(camera_position))
    , look_at(std::move(look_at))
...
void Camera::end_drag() {
    is_drag = false;
}

void Camera::update_drag(const glm::vec2& mouse_pos, const glm::vec2& view_size) {
    if (!is_drag) {
        return;
    }
    ...
    if (cosine_angle * sgn(delta.y) > 0.99f) { delta.y = 0; }

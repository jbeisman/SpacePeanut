...
fft_util::create_plans(const int NGRID, float *mass, float *potential,
  ...
  c2r_plan = fftwf_plan_dft_c2r_3d(NGRID, NGRID, NGRID, mass_fft, potential,
                                   FFTW_ESTIMATE);
...
void fft_util::destroy_plans(fft_util::Plans *plans) {
  fftwf_destroy_plan(plans->r2c_plan);
  fftwf_destroy_plan(plans->c2r_plan);
}

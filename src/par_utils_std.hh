...
void mult_vec_par(T& vec,
	const float val)
{
	std::transform(std::execution::par_unseq, vec.begin(), vec.end(), vec.begin(), [val](auto el) { 
		el *= val;
		return el;
	});
}
...
void mult_vec3_par(
	...
	const float val)
{
 	std::transform(std::execution::par_unseq, vec3.begin(), vec3.end(), vec3.begin(), [val](auto v) { 
		v[0] *= val;
    	v[1] *= val;
    	v[2] *= val;
    	return v;
...
{
	std::transform(std::execution::par_unseq, inVec3.begin(), inVec3.end(), outVec3.begin(), outVec3.begin(),
	...
	U& outVec)
{
	...
	std::transform(std::execution::par_unseq, inVec3.begin(), inVec3.end(), outVec.begin(),
    [ts2_inv](const auto& vel) {

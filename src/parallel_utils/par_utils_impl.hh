...
template <typename T>
void update_particle_par(const float const_factor, const T &inVec3, T &outVec3);
...
template <typename T> void assign_vec(T &vec, const float val) {
  assign_vec_par(vec, val);
}

template <typename T> void mult_vec(T &vec, const float val) {
  mult_vec_par(vec, val);
}

template <typename T> void mult_vec3(T &vec3, const float val) {
  mult_vec3_par(vec3, val);
}

template <typename T>
void update_particle(const float const_factor, const T &inVec3, T &outVec3) {
  update_particle_par(const_factor, inVec3, outVec3);
}

template <typename T, typename U>
void magnitude_vec3(const float time_scale, const T &inVec3, U &outVec) {
  magnitude_vec3_par(time_scale, inVec3, outVec);
}

template <typename T> std::pair<float, float> minmax_vec_elems(const T &inVec) {

...
void compute_grid_potential(
  ...
  const float kfac = std::numbers::pi_v<float> * std::numbers::pi_v<float> *
                     4.0 / (grid.GMAX * grid.GMAX);
  ...
  const float normfactor = 4.0 * G() * std::numbers::pi_v<float> /
                           (time_scale * (float)(NG * NG * NG));

...
2. compute_grid_potential() computes the grid gravitational potential field from
the grid mass distribution.
3. compute_grid_acceleration() calculates the grid acceleration field from the
grid potential field.
4. compute_particle_acceleration() interpolates the grid acceleration field onto
the particles.
*/
void compute_grid_mass_distribution(
    const GridConst &grid, const std::vector<std::array<float, 3>> &pPos,
    std::vector<float, fftwf_allocator<float>> &gMass);

void compute_grid_potential(
    const GridConst &grid, const fft_util::Plans *fft_plans,
    const float time_scale, const std::vector<std::array<short int, 3>> &fft_i,

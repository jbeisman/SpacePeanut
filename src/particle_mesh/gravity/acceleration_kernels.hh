...
inline void kernel_grid_mass_CIC(float CVOL_INV,
                                 const std::array<float, 3> &pos, float *gMD) {
  ...
  gMD[GridIDX::flatten(iX + 1, iY + 1, iZ)] +=
      (10.0 * fXX * fYY * fZ * CVOL_INV);
  gMD[GridIDX::flatten(iX + 1, iY, iZ + 1)] +=
      (10.0 * fXX * fY * fZZ * CVOL_INV);
  gMD[GridIDX::flatten(iX, iY + 1, iZ + 1)] +=
      (10.0 * fX * fYY * fZZ * CVOL_INV);
  gMD[GridIDX::flatten(iX + 1, iY + 1, iZ + 1)] +=
      (10.0 * fXX * fYY * fZZ * CVOL_INV);
}
...
inline void kernel_grid_potential(int NG, int NGH, float kfac, int i, int j,
  ...
  float k_squared_inv = (k_squared < std::numeric_limits<float>::epsilon())
                            ? 0.0f
                            : 1.0f / k_squared;
...
// Calculate grid acceleration from gradient of potential via 4th-order central
// finite difference
inline void kernel_grid_accel(int i, int j, int k, float dnom, const float *gP,
                              std::array<float, 3> &gA) {
  gA[0] = dnom * (gP[GridIDX::flatten(i - 2, j, k)] -
                  8 * gP[GridIDX::flatten(i - 1, j, k)] +
                  8 * gP[GridIDX::flatten(i + 1, j, k)] -
                  gP[GridIDX::flatten(i + 2, j, k)]);
  gA[1] = dnom * (gP[GridIDX::flatten(i, j - 2, k)] -
                  8 * gP[GridIDX::flatten(i, j - 1, k)] +
                  8 * gP[GridIDX::flatten(i, j + 1, k)] -
                  gP[GridIDX::flatten(i, j + 2, k)]);
  gA[2] = dnom * (gP[GridIDX::flatten(i, j, k - 2)] -
                  8 * gP[GridIDX::flatten(i, j, k - 1)] +
                  8 * gP[GridIDX::flatten(i, j, k + 1)] -
                  gP[GridIDX::flatten(i, j, k + 2)]);
}

// Cloud-in-cell interpolation of acceleration onto particles
inline void kernel_part_accel_CIC(const std::array<float, 3> &pos,

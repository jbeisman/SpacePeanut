...
void compute_grid_potential_par(
  ...
  std::for_each(std::execution::par_unseq, fft_i.begin(), fft_i.end(),
                [NG, NGH, kfac, fMass](const auto &ijk) {
                  auto [i, j, k] = ijk;
                  kernel_grid_potential(NG, NGH, kfac, static_cast<int>(i),
                                        static_cast<int>(j),
                                        static_cast<int>(k), fMass);
                });
}
...
void compute_grid_acceleration_par(
  ...
  std::transform(std::execution::par_unseq, grid_i.begin(), grid_i.end(),
                 gAcc.begin(), gAcc.begin(),
                 [dnom, gP](const auto &ijk, auto &gA) {
                   auto [i, j, k] = ijk;
                   kernel_grid_accel(static_cast<int>(i), static_cast<int>(j),
                                     static_cast<int>(k), dnom, gP, gA);
                   return gA;
                 });
}
...
void compute_particle_acceleration_par(
  ...
  std::transform(std::execution::par_unseq, pPos.begin(), pPos.end(),
                 pAcc.begin(), pAcc.begin(), [gA](const auto &pos, auto &pA) {

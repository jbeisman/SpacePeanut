...
std::tuple<int, int, int, float, float> get_input() {
  ...
  auto return_default = [=]() {
    return std::make_tuple(default_nbods, default_ngrid, default_nsteps,
                           default_gmax, default_rshft);
  };
  ...
  if (user_choice == "y") {
    ...
    std::cout << "Enter on a single line separated by spaces, eg. 2097152 256 "
                 "1000 128.0 50.0 \n\n";
    ...
    if (ss >> nbods >> ngrid >> nsteps >> gmax >> redshift) {
      ...
      std::cerr << "Error: Could not parse input. Returning default values."
                << std::endl;
  ...
  std::cerr
      << "Invalid choice. Please enter 'y' or 'n'. Returning default values."
      << std::endl;
  return return_default();
}

...
struct CosmicTime {
  ...
  float get_end_time() const { return t_end; }

  float get_time_scale() const { return exp(static_cast<float>(time)); }

  void update_time(double val) { time += val; }

  void update_dt(float val) { dt = val; }
...
public:
  ...
  ~ParticleMeshSimulator();
  void initialize_simulation(const float redshift, const int ntimesteps,
